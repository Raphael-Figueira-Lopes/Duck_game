...
class Level extends Phaser.Scene {
		...
		if(this.lastFired < (this.time.now - 1000)){
			...
			this.ducks.children.iterate(child => {
				if (child && child.getBounds().contains(this.crosshair.body.position.x, this.crosshair.body.position.y)) {
				  ...
				  this.scoreText(this.score += 10, 1); 
				}
			})
	...
	//visualize the score like image (i need do it)
	scoreText(score, where){ 
		let scor = score.toString().split("");
		...
		{ spaceImage += 50;
			const scoreImg = this.add.image(150 + spaceImage, 64, "text_"+scor[i]);
			this.textgroup.add(scoreImg);
